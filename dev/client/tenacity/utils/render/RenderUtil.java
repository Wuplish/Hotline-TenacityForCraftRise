...
public class RenderUtil implements Utils
{
    ...
    public static be createFrameBuffer(be framebuffer)
    {
        if (framebuffer == null || framebuffer.i != sr.getScaledWidth() || framebuffer.d != sr.getScaledHeight())
        {
            if (framebuffer != null)
            {
                framebuffer.c();
            ...
            return new be(sr.getScaledWidth(), sr.getScaledHeight(), true);
        }
    ...
    public static void drawGoodCircle(double x, double y, float radius, int color)
    {
        ...
        GLUtil.setup2DRendering(() ->
        {
            ...
            glPointSize(radius * (2 * Config.getMinecraft().ap.z));
