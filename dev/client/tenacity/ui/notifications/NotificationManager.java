...
public class NotificationManager
{
    ...
    public void drawNotifications(ScaledResolution sr)
    {
        ...
        for (Notification notification : notifications)
        {
            ...
            if (count == 0)
            {
                notification.notificationY = sr.getScaledHeight();
            }
            ...
            {
                notifX += notifWidth * notification.getAnimation().getOutput();
            }
    ...
    public void notificationDraw(float x, float y, float width, float height, Notification notification)
    {
        ...
        Color colorr = ColorUtil.interpolateColorC(baseColor, new Color(ColorUtil.applyOpacity(color, .3f)), NotificationsMod.colorInterpolation.getValue().floatValue());
    ...
    public void blurNotifs(ScaledResolution sr)
    {
        ...
        for (Notification notification : notifications)
        {
            ...
            if (count == 0)
            {
                notification.notificationY = sr.getScaledHeight();    //- Watermark.y;
            }
